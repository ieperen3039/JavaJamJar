...
public abstract class AbstractMap extends StaticEntity implements GameMap {
    ...
    private static final FixedState GROUNDED_STATE = new FixedState(new Vector3fx(), new Quaternionf());

    public AbstractMap() {
        super(GROUNDED_STATE);
    }
    ...
    public boolean checkMouseClick(MouseTool tool, int xSc, int ySc, Game game) {
        ...
        float t = this.getIntersection(origin, direction);
        if (t == 1) return false;
    ...
    public float intersectFractionBoundingBox(
        ...
        for (Vector3f corner : hitbox.corners()) {
            ...
            float newIntersect = this.getIntersection(corner, direction);
